...
Jami name service is used to resolve easily memorable usernames to Jami IDs.
Jami clients communicate with the name service using HTTP.
...
You'll need to use Python 3.5+ to run the nameservice.
If you don't already have tmux installed, do `pip install tmux`, and then:
...
Send a GET request to `name/<username>` to get its Jami ID.
Send a POST request to `name/<username>` to register a new username-JamiID pair.
